...
class deep_q_agent:
    ...
    def _train(self):
        ...
        for i in range(self.batch_size):
            ...
            action_idx1 = action_space_map[str(action_idx) + str(int(temp_actions[i+1]))]

            if temp_terminal[i+1]:
                target[i][action_idx1] = temp_rewards[i+1]
            else:
                target[i][action_idx1] = temp_rewards[i] + 0.99 * temp_rewards[i+1] + 0.99 * 0.99 * np.amax(q_values_train_next[i+1])
